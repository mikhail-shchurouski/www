...
if ( $_SESSION['auth'] == 'yes_auth' ) 
 ...
 {
$_SESSION["order_delivery"] = $_POST["order_delivery"]; // сохраняем в сессию все данные введенные в поля для доставки товара 
...
<?php                                               //РЕАЛИЗУЕМ ОФОРМЛЕНИЕ ЗАКАЗА В КАРЗИНЕ В 3 ШАГА
	$action = clear_string($_GET["action"]);     // проверка переменной action - в зависимости какой case в нее попадает - такой блок кода и выполняется.
    
    switch($action){                              
        
        
        case 'oneclick':                                         // здесь реализуем вывод товаров добавленных в корзину
...
// выводим блок с ценой картинкой наименованием того что лежит в корзине
echo '                                          
...
';
 ...
 echo '
 <h2 class="itog-price" align="right">Итого: <b>'.group_numerals($itogpricecart).'</b> руб.</h2>
...
break;
        ...
        case 'confirm':
                                                           // здесь будет информация о доставке 2-ой шаг при оформлении
          echo'      
        <div id = "block-step">                     
        <div id = "name-step">        
        <ul>
...
if ($_SESSION['order_delivery'] == "По почте") $chck1 = "checked"; // проверяем сессии на тот случай если клиент вернется назад опции выбранные ранее сохранятся
...
<ul id="info-order">
';
  if ( $_SESSION['auth'] != 'yes_auth' ) // если пользователь автвризирован то форму выводить не нужно
{
echo '
<li><label for="order_fio"><span>*</span>ФИО</label><input type="text" name="order_fio" id="order_fio" value="'.$_SESSION["order_fio"].'" /><span class="order_span_style" >Пример: Сидоров Иван Петрович</span></li>
<li><label for="order_email"><span>*</span>E-mail</label><input type="text" name="order_email" id="order_email" value="'.$_SESSION["order_email"].'" /><span class="order_span_style" >Пример: sidorov@gmail.com</span></li>
<li><label for="order_phone"><span>*</span>Телефон</label><input type="text" name="order_phone" id="order_phone" value="'.$_SESSION["order_phone"].'" /><span class="order_span_style" >Пример: 8 029 100 12 34</span></li>
<li><label class="order_label_style" for="order_address"><span>*</span>Адрес<br /> доставки</label><input type="text" name="order_address" id="order_address" value="'.$_SESSION["order_address"].'" /><span>Пример: г. Минск,<br /> ул. Есенина д. 18, кв. 58</span></li>
';
}
echo '
...
<p align="right" ><input type="submit" name="submitdata" id="confirm-button-next" value="Далее" /></p>
 ...
 ';
        ...
        case 'completion':                    // завершающий шаг при оформлении
        ...
        if ( $_SESSION['auth'] == 'yes_auth' )  // проверяем ,если пользователь зарегистрирован то выводим данные введенные при регистрации
    {                                           // в противном случае - выводим данные из заполненой формы на шаге 2
echo '
<ul id="list-info" >
...
';
 ...
 echo '
 <h2 class="itog-price" align="right">Итого:<b> '.group_numerals ($all_price).'</b> руб.</h2>
 <p align="right" class="button-next" ><a href="cart.php?action=confirm" >Оплатить</a></p> 
 ';
