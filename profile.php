...
{	
    ...
    include("functions/functions.php");
     ...
     if ($_POST["save_submit"]) // реализация изменеия данных профиля если все поля заполнены и все проверки сработали в тру жмем сохранить и в базе делается замена данных
     {
        
    $_POST["info_surname"] = clear_string($_POST["info_surname"]);    // проверяем есть ли что в полях для заполнения
    ...
    $pass   = md5($_POST["info_pass"]);                  // здесь в поле текущий пароль сравниваем текущий пароль с тем что ввел пользователь и проверяем не ошибся ли он
    $pass   = strrev($pass);
	...
	}else
    {
      ...
      if($_POST["info_new_pass"] != "")          // здесь смотрим что делается в поле новый пароль, если оно пустое значит оставляем старый пароль если нет старый заменяем новым
	{
		        if(strlen($_POST["info_new_pass"]) < 7 || strlen($_POST["info_new_pass"]) > 15)
            	{
		           $error[]='Введите новый пароль от 7 до 15 символов!';    // проверяем длину пароля
	            }else
                {
                     $newpass   = md5(clear_string($_POST["info_new_pass"])); // шифруем новый пароль
        ...
        if(strlen($_POST["info_surname"]) < 3 || strlen($_POST["info_surname"]) > 15) // проверяем поле для ввода фамилии длинна строк и т д
	{
		$error[]='Укажите фамилию от 3 до 15 символов!';
	}
    
    
        if(strlen($_POST["info_name"]) < 3 || strlen($_POST["info_name"]) > 15) // проверка имни
	{
		$error[]='Укажите имя от 3 до 15 символов!';
	}
    
    
        if(strlen($_POST["info_patronymic"]) < 3 || strlen($_POST["info_patronymic"]) > 25) // проверка отчества
	{
	...
	{
		$error[]='Укажите корректный email!'; // проверка почты
	}
    
      if(strlen($_POST["info_phone"]) == "")
	{
		$error[]='Укажите номер телефона!'; // проверка телефона
	} 
    
      if(strlen($_POST["info_address"]) == "")
	{
		$error[]='Укажите адрес для доставки!'; // проверка адреса
	}      
  ...
  if(count($error)) 
	{
		$_SESSION['msg'] = "<p align='left' id='form-error'>".implode('<br />',$error)."</p>";  //если есть ошибки в форме выводим ошибки
	}else
    {
        $_SESSION['msg'] = "<p align='left' id='form-success'>Данные успешно сохранены!</p>";// если ошибок нет выполняем сохранение
     ...
     $update = mysql_query("UPDATE reg_user SET $dataquery WHERE login = '{$_SESSION['auth_login']}'",$link); // и если сохранение делаем то выполняем запрос в базу данных уапдейт - заменяем данные в полях старые на новые
      
    if ($newpass){ $_SESSION['auth_pass'] = $newpass; }  // проверка если новый пароль в базе данных есть - то добавляем его  в сессию
...
<li>
<label for="info_address">Адрес для доставки</label>
<span class="star">*</span>
