<?php      
   ...
   include("include/db_connect.php"); // реализация поиска товаров по сайту
   include("functions/functions.php");
   session_start();
   include("include/auth_cookie.php");
   
$search = clear_string($_GET["q"]);   // очищаем поисковое значение и принимаем переменную q 
...
<?php
  ...
  if (strlen($search) >= 3 && strlen($search) < 150) // количество символов в поисковом значении
  {    
    ...
    $temp = mysql_fetch_array($count);// делаем запрос к базе данных вывести все товары разрешенные к показу где есть совпадение с запросом LIKE
	If ($temp[0] > 0)
	{  
	$tempcount = $temp[0];
	...
	// Вычисляем с кокого номера следует выводить товары
...
<p id="nav-breadcrumbs"><a href="index.php">Главная станица</a> | <span> Поиск</span></p>
<ul id="option-list">
<li>Вид:</li>
...
// Формируем ссылки со страницами постраничная навигация
